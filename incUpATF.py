...
if __name__ == '__main__':
    ...
    if input_dir:  # Update from directory
        ...
        c = db.cursor()
        sql = 'select id,location from images where id > ' + str(
            startid) + endid_str + ' and location is not null order by id asc limit ' + str(limit)
        query_id = []
    ...
    for i in range(0, len(unique_sha1)):
        if unique_sha1[i] not in old_uniques:
            ...
            if img_item[0]:
                ...
                new_uniques.append((umax + i, location, img_item[3]))
            #new_id = new_id + 1
    ...
    if num_new_unique:
        ...
        flog.write('feature extraction image_number: ' + str(ins_num) + ' batch_size: ' + str(
            batch_size) + ' iteration: ' + str(iteration) + '\n')
        ...
        f.write(proto)
        f.close()
        command = './extract_nfeatures_gpu caffe_sentibank_train_iter_250000 ' + protoname + ' fc7 ' + featurename + '_fc7 ' + str(
            iteration) + ' ' + device;
        print command
        flog.write(command + '\n')
        os.system(command)
    ...
    if not success:
        ...
        flog.write(update_suffix + 'Update is success!' + '\n' + 'Total time: ' + str(
            time.time() - step_times[0]) + ' seconds\n')
        flog.close()
        # post processing
        # delete img cache
