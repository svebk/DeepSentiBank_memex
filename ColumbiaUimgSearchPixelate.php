...
$fout = fopen ($outname, "rb");
 if ($fout) {
    ...
    //echo '<div id="debug" value="'.time_elapsed(time()-$start_time).'" params="image_url:'.$image_url.';query_num:'.$query_num.';vis:'.$vis.';fast:'.$fast.';nodup:'.$nodup.';neardup:'.$neardup.';neardup_th:'.$neardup_th.';nocache:'.$nocache.';"></div>';
		echo '<font size="6"><b>Query Image</b></font><br><a href="'.$image_url.'"><img src="'.$image_url.'" style="margin:3;border:1;height:120px;" title="Query Image" data-pixelate data-value="0.2" data-reveal="false"></a><br><br><font size="6"><b>Query Results:</b><br>';
		...
		for ($i=0; $i<sizeof($imglist); $i++) {
			...
			echo '<a href="'.$dupurl.'"><img src="'.$imglist[$i].'" style="margin:3;border:1;height:120px;" origin="'.$orilist[$i].'" title="'.$distlist[$i].'" data-pixelate data-value="0.2" data-reveal="false"></a>';
		}
	}
